...
): MiniTimelineProps | any => ({
  onNewCurrentTime: (time: number) => {
    dispatch(trackActions.setAudioNewTime(time));
  },
  onNewAudioStatus: (status: boolean) => {
    dispatch(trackActions.setAudioStatus(status));
  },
});
...
> {
  ...
  handleActiveBoxDrag = (event: DraggableEvent, data: DraggableData): void => {
    let offset =
      -(data.x * ((this.props.duration * 7) / 840)) /
      ((this.props.duration * 7) / 840);
    this.setState({
  ...
  handleActiveBoxEnd = (event: DraggableEvent, data: DraggableData): void => {
    ...
    const newTime =
      this.props.currentTime + (delta / 7) * ((this.props.duration * 7) / 840);
    this.props.onNewCurrentTime(newTime);
    this.props.onNewAudioStatus(true);
  };

  componentDidUpdate(
    prevProps: MiniTimelineProps,
    prevState: MiniTimelineState
  ) {
    if (this.props.currentTime !== prevProps.currentTime)
      this.setState({
        currentTimeOffset:
          (this.props.currentTime * 7) / ((this.props.duration * 7) / 840),
      });
  }
    ...
    return (
      <div className="mini-timeline-container">
        <div className="draggable-container">
          ...
          >
            <div
              className="mini-timeline-active-box"
              style={setActiveBoxTransition()}
            ></div>
          </Draggable>
        </div>
...
const MiniTimelineContainer: ConnectedComponent<
  typeof MiniTimeline,
  any
> = connect(mapStateToProps, mapDispatchToProps)(MiniTimeline);
